...
class SklearnNN(object):
    def __init__(self, num_neighbors: int = 5, num_workers: int = 4) -> None:
        """Nearest neighborhood search using sklearn.

        Args:
            num_neighbors: Number of neighbors to use with NearestNeighbors.
            num_workers: Number of parallel jobs to run for neighbors search.
        """
        self.num_neighbors = num_neighbors
        self.num_workers = num_workers
        self.search_index = None

    def _create_index(self, n_neighbors: int, algorithm: str = 'auto'):
        return NearestNeighbors(n_neighbors=n_neighbors, n_jobs=self.num_workers, algorithm=algorithm)

    def fit(self, features: np.ndarray) -> None:
        """
        Adds features to the NearestNeighbors search index.

        Args:
            features: Array of size NxD.
        """
        self.search_index = self._create_index(self.num_neighbors)
        self.search_index.fit(features)

    def run(
            self,
            n_nearest_neighbors: int,
            query_features: np.ndarray,
            index_features: Optional[np.ndarray] = None,
    ) -> Union[np.ndarray, np.ndarray]:
        """
        Returns distances and indices of nearest neighbor search.

        Args:
            n_nearest_neighbors: Number of nearest neighbors to retrieve.
            query_features: Features to retrieve.
            index_features: [optional] Index features to search in.
        """
        if index_features is not None:
            # Build a temporary search index for this search.
            temp_index = self._create_index(n_nearest_neighbors)
            temp_index.fit(index_features)
            distances, indices = temp_index.kneighbors(query_features, n_neighbors=n_nearest_neighbors)
        else:
            distances, indices = self.search_index.kneighbors(query_features, n_neighbors=n_nearest_neighbors)

        return distances, indices

    def save(self, filename: str) -> None:
        import joblib
        joblib.dump(self.search_index, filename)

    def load(self, filename: str) -> None:
        import joblib
        self.search_index = joblib.load(filename)
